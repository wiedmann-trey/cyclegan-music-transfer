...
def pretrain(epochs=35, vocab_size=391, save=True, load=True):
    ...
    if load:
        ...
        model.load_state_dict(torch.load("pretrain_classical_jazz/35_pretrain_classical_jazz.pth", map_location=device))
    model = model.to(device)
...
def train(epochs=20, vocab_size=391, save=True, load=True):
    ...
    if load:
        model.load_state_dict(torch.load("trained_pop_jazz/76train_model.pth", map_location=torch.device(device)))
    model = model.to(device)
