...
class Discriminator(nn.Module):
    ...
    def __init__(self, vocab_size=357, embedding_dim=256, hidden_dim=256):
...
class Encoder(nn.Module):
    def __init__(self, vocab_size=357, embedding_dim=256, hidden_dim=256, dropout_rate=.1):
...
class CycleGAN(nn.Module):
        ...
        def forward(self, real_A, real_B, x_m):
            ...
            if self.mode == 'train':
                ...
                #DA_fake_all = self.D_A_all(sample_fake_A)
                #DB_fake_all = self.D_B_all(sample_fake_B)
